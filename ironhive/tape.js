...
window.Tape = (function () {
  ...
  const store = Lab.store('tape', () => ({
    ...
    // without being in the road — which now means the gutter BETWEEN two
    // rows of features rather than across the first one. The tape lies over
    // the features (see .tape-layer in lab.css), so anywhere on top of one
    // is very much in the road.
    strips: [{ id: 'tape-1', x1: 60, y1: 1100, x2: 700, y2: 1088 }]
  }));
