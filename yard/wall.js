...
window.Wall = (function () {
  ...
  function add(item) {
    mark('ink');
    // YARD: when it went on. The dashboard's heat, trend and 'your edits' read
    // this; the bench's own wall.js does not carry it. `when`, because a note's
    // `t` is its text and a gif's `at` is where it starts. (2026-09-17)
    if (item && item.when == null) item.when = Date.now();
  ...
  svg.addEventListener('pointerdown', e => {
    ...
    if (tool === 'draw') {
      ...
      /* …AND A DRAG CARRIES IT (2026-09-22). Press a note and move, and it goes where you take
         it, the way the move tool would, without the text tool being put down; let go where you
         pressed and you are writing it. The sticker tool's referee (armGrab), given the note's
         own ending and no clock: holding a note still is reading it, not picking it up. */
      const n = i >= 0 && byIndex(i);
      if (n) armGrab(n, e, () => editNote(i));
      else onTap(e, () => askNote(p));
    } else if (tool === 'gif') {
  ...
  function armGrab(t, e, fn) {
    grabOff();                                   // a second press supersedes the first
    const p = W(e.clientX, e.clientY);
    grab = { t: t, id: e.pointerId, x: p.x, y: p.y, cx: e.clientX, cy: e.clientY, ev: e, timer: 0, fn: fn };
    // a note's press (the text tool's, fn: open it) has no clock: see there
    if (!fn) grab.timer = setTimeout(take, GRAB_MS);
  ...
  function grabMove(e) {
    ...
    const p = W(e.clientX, e.clientY);
    // …and its slop is in screen px, so a click that shivers still opens the note at any zoom
    if (grab.fn ? Math.abs(e.clientX - grab.cx) + Math.abs(e.clientY - grab.cy) > NOTE_SLOP
                : Math.abs(p.x - grab.x) > GRAB_SLOP || Math.abs(p.y - grab.y) > GRAB_SLOP) take();
  }

  function grabEnd(e) {
    if (!grab || (e && e.pointerId !== grab.id)) return;
    const g = grabOff();
    if (g.fn) { if (e.type !== 'pointercancel') g.fn(); return; }   // a note let go where it was pressed: open it
    const s = window.Stickers && Stickers.armed();
  ...
  function askNote(p, ed) {
    ...
    hint.textContent = 'enter to pin it · drag the edge to move it · shift+enter for a new line · ctrl b/i/u · ' +
  ...
  function styleNote() {
    ...
    if (noteBox) noteBox.classList.toggle('wall-note-dark', !bg && paleInk());
    grow();
  }
      ...
      store.update(st => {
        ...
        if (vid) {
          st.items[ed.i] = Object.assign(vid,
            { x: round((p.moved ? p.x + 10 : old.x) + VID[0] / 2), y: round(p.moved ? p.y : old.y) },
  ...
  opts.addEventListener('click', e => {
    ...
    const b = e.target.closest('[data-sk],[data-po],[data-fo],[data-px],[data-bl],[data-z],[data-ty],[data-al],[data-iz],[data-bg]');
    if (!b) return;
