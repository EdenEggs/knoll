...
window.Tracer = (function () {
  ...
  function file() {
    ...
    let kept = false;
    try { kept = (localStorage.getItem(Lab.prefix + 'flatfile') || '').indexOf('"' + id + '"') >= 0; } catch (e) {}
    if (!kept) {
  ...
  function wireDrag(box) {
    ...
    const end = e => {
      ...
      // over the paper, and not over the table itself (the yard's paper: see stickers.js)
      const t = document.elementFromPoint(e.clientX, e.clientY);
      if (!t || t.closest('#tracer') || !t.closest('#bench, #bench-world, .yard-root')) { say('let go over the paper to stamp it'); return; }
