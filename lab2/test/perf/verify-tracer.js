/* lab2/test/perf/verify-tracer.js — lab2/perf/verify-tracer.js run against the
   SANDBOX bench (lab2/test/ on 4322, its tracer calling press/palette.js), the
   URL and the door block being the only changes; it must say 12/12 exactly as
   the live one does. The door is blocked wide ('**\/_lab2/**') as every sandbox
   harness blocks it, and the pictures land under results/verify-tracer/.
   THE LIVE HEADER FOLLOWS. — the tracing table's library, since 2026-09-04:
...
const OUT = path.join(__dirname, 'results', 'verify-tracer');
