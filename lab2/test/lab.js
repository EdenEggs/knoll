...
window.Lab = (function () {
  ...
  /* every localStorage key on this page starts with NS — the bench's own
     'knoll-lab2:', or 'knoll-lab2:<page>:' on a page that names itself.
     Read once, here, because KEY is the first key made; the argument is
     PAGE-SCOPED STORES, down in the store section. */
  const PAGE = (document.documentElement.dataset.labPage || '').trim();
  const NS = 'knoll-lab2:' + (PAGE ? PAGE + ':' : '');
  const KEY = NS + 'pos:';
  const gizmos = [];
  ...
  function paste() {
    ...
    clip.forEach(c => {
      ...
      if (c.cut) { try { localStorage.setItem(NS + 'size2:' + e.id, JSON.stringify(c.cut)); } catch (err) {} }
  ...
  function deleteCopy(el) {
    ...
    remember(() => {
      const back = makeGz(e);
      if (!back) return;
      if (cut) { try { localStorage.setItem(NS + 'size2:' + e.id, JSON.stringify(cut)); } catch (err) {} }
  ...
  function shelve(el) {
    ...
    let size = null;
    try { size = JSON.parse(localStorage.getItem(NS + 'size2:' + el.dataset.gizmo)); } catch (e) {}
    const z = zOf(el);
  ...
  function unhide(r) {
    ...
    if (r.cut) { try { localStorage.setItem(NS + 'size2:' + r.id, JSON.stringify({ w: r.w, h: r.h })); } catch (e) {} }
    world.appendChild(el);
    register(el);
  ...
  const CAM_KEY = NS + 'cam', OLD_ZKEY = NS + 'zoom';
  ...
  /* PAGE-SCOPED STORES. localStorage is per ORIGIN, and a game page
     (lab2/games/<slug>/, built by the Press Table) is served from the same
     origin as this bench — so, keyed the same way, the two would share one
     camera, one set of positions, one wall and one tape: drag a sticker on
     a game page and the bench's tree of the same name moves too (verified
     on 4321 in Phase 0, press/NOTES.md). So a page that names itself —
     <html data-lab-page="games/<slug>"> — gets every key it writes, and
     every key a gizmo's store writes, under 'knoll-lab2:<page>:' instead
     of 'knoll-lab2:'; NS, at the top of this file, is that prefix, read
     once. A page with no name is the bench, and its keys are exactly what
     they were. storeKey(k) hands the prefix to frames.js (its size key)
     and keep.js (the keys it clears after a save), which is every other
     file that spells a key out rather than opening a store here. */
  const storeKey = k => NS + k;
  const stores = [];
  ...
  /* ── WHERE IT OPENS ─────────────────────────────────────────────────────
     ...
     A PAGE MAY WRITE ITS OWN RECTANGLE, since 2026-09-07. A generated game
     page (lab2/games/<slug>/, built by the Press Table) frames its own
     composition — the key art, the notes, the stickers stood round them —
     and its four numbers are the generator's, argued for in that file and
     not in this one. So #bench-world may carry data-open="x,y,w,h" for a
     wide screen and data-open-narrow="x,y,w,h" for a narrow one, and they
     are read first, below; the two literals stand behind them. An attribute
     that is absent, or is not four finite numbers with a width and a height
     above zero, is ignored and the literal is used. The main bench carries
     neither attribute and opens exactly where it did. */
  if (!restored) {
    ...
    /* …OR THE PAGE'S OWN, off #bench-world — A PAGE MAY WRITE ITS OWN
       RECTANGLE, above. Four numbers, comma-separated, in world units. The
       narrow one falls back to the wide ATTRIBUTE before it falls back to
       the literal: a page that wrote one rectangle opens on it at every
       width, and a page that wrote none is the main bench. */
    const rectAttr = (name, fallback) => {
      const v = String(world.dataset[name] || '').split(',').map(Number);
      return v.length === 4 && v.every(isFinite) && v[2] > 0 && v[3] > 0
        ? { x: v[0], y: v[1], w: v[2], h: v[3] } : fallback;
    };
    const OPEN_WIDE = rectAttr('open', WIDE), OPEN_NARROW = rectAttr('openNarrow', rectAttr('open', NARROW));
    const b = box(), phone = b.width < 700;
    const HOME = phone ? OPEN_NARROW : OPEN_WIDE, pad = phone ? 16 : 24;
  ...
  return { register, place, resetAll, gizmos, store, storeKey, uid, resetData, world, bench, grid, growBench,
