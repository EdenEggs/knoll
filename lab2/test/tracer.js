/* ─── THE TRACING TABLE ───────────────────────────────────────────────────
   ...
   0–5 the pipeline was tuned at. Since 2026-09-07 the clustering itself
   (the sampler and the k-means) lives in press/palette.js as Palette.sample
   and Palette.kmeans, because the Press Table reads a game's key art with
   the same quantiser and one copy cannot drift from another — the same
   code, moved, and this table calls it there; perf/golden-tracer.js is the
   proof that a tracing came out the same to the byte.
...
window.Tracer = (function () {
  ...
  // SAMPLE_CAP (3000 pixels sampled for k-means) went to press/palette.js with the sampler; the full image is only ever nearest-assigned, here
  ...
  // sampleOpaque and kMeansPalette are Palette.sample and Palette.kmeans in
  // press/palette.js now (the same code, moved — the Press Table quantises
  // with them too); labelPixels stays, since the full image is only ever
  // nearest-assigned here, for the masks.
  ...
  function vectorize(imageData, K, smooth) {
    if (!window.Palette) { flash('the palette module is missing (press/palette.js)'); return null; }
    const { data, width: w, height: h } = imageData;
    const samples = Palette.sample(imageData);
    if (!samples.length) return null;

    const centroids = Palette.kmeans(samples, K);
  ...
  function runTrace() {
    ...
    setTimeout(() => {
      try {
        ...
        if (!result) {
          autoFile = false;
          if (window.Palette) say('nothing to trace — that picture looks fully transparent');   // else vectorize() has said what is missing
  ...
  function file() {
    ...
    let kept = false;
    // the store's own key, under the page's prefix when it has one (lab.js,
    // PAGE-SCOPED STORES) — read back raw, since the store's save may have
    // swallowed a quota error and the state in memory would not say so
    const K = Lab.storeKey ? Lab.storeKey('flatfile') : 'knoll-lab2:flatfile';
    try { kept = (localStorage.getItem(K) || '').indexOf('"' + id + '"') >= 0; } catch (e) {}
    if (!kept) {
