/* ── KEEP: THE BENCH SAVES ITSELF AS THE DEFAULT LOOK ──────────────────────
   ...
   knoll-lab2:pos:<gizmo> and frames.js keeps re-cut sizes in
   knoll-lab2:size2:<gizmo>, and both of those are IN THIS BROWSER — they are
   ...
   WHICH DOOR is read off the address — three lines, no shared file, the
   same three in press.js, argued for in press/CONTRACTS.md §0. The folder
   the page was served from (the path up to its last '/') is matched for
   the game-page shape, /lab2<prefix>/games/<slug>/, which knocks on
   /_lab2<prefix>/games/<slug>; anything else that is /lab2<prefix>/ or
   /lab2<prefix>/press/ knocks on /_lab2<prefix>/default. So the bench at
   /lab2/ knocks on /_lab2/default exactly as it always did, a game page
   knocks on its own door, and the sandbox at /lab2/test/ knocks on
   /_lab2/test/default — never on the live bench's, which is what a
   hard-coded door would have done from there. Deployed, at /, it is
   /_lab2/default again, which 404s as it always did. The keys this file
   clears after a save are the page's own too: lab.js prefixes every
   localStorage key with data-lab-page when a page carries one
   (Lab.storeKey), so a game page's positions and the bench's never answer
   to one name.
   ...
   WHAT IT SAVES
     ...
     · a sticker's own words ................... data-palette, data-text,
                                                  data-rot (kits.js)
...
window.Keep = (function () {
  ...
  /* WHICH DOOR — see the header. The folder this page came from, then the
     game-page shape, then the bench's; CONTRACTS.md §0 is where the three
     lines are argued, and press.js carries the same three. */
  const dir = location.pathname.replace(/[^/]*$/, '');
  const game = /^\/lab2(\/.*?)?\/games\/([a-z0-9-]+)\/$/.exec(dir);
  const DOOR = game ? '/_lab2' + (game[1] || '') + '/games/' + game[2]
                    : '/_lab2' + ((/^\/lab2(\/.*?)??\/(?:press\/)?$/.exec(dir) || [])[1] || '') + '/default';
  // the page's own keys — lab.js's prefix when it has one, the bench's when
  // it is a lab.js from before there was one
  const skey = k => (window.Lab.storeKey ? window.Lab.storeKey(k) : 'knoll-lab2:' + k);
  ...
  function look() {
    ...
    return window.Lab.gizmos.map(g => {
      ...
      return {
        ...
        /* THE STICKER'S OWN WORDS (kits.js; CONTRACTS.md §7): a per-section
           palette, the words in a text slot, a rotation. Always sent, as
           strings, and '' does not mean "nothing to say" — it means TAKE
           THE WORD OFF: serve.js drops an attribute for an empty value and
           leaves one alone only for a key it was never sent, so a palette
           cleared on the bench is cleared in the file, and a client that
           did not know the word could not have removed it. */
      ...
      palette: r.el.dataset.palette || '', text: r.el.dataset.text || '', rot: r.el.dataset.rot || ''
    })));
  }
  ...
  const stamp = list => JSON.stringify(list.map(g =>
    /* …and the three words ride along, so a palette or a caption changed on
       the bench is a change the timer saves. An array rather than the joined
       string it used to be, because a caption may carry a comma of its own. */
    [g.gizmo, g.x, g.y, g.cut ? g.w : 0, g.cut ? g.h : 0, g.z, g.gone ? 1 : 0, g.palette, g.text, g.rot]));
  ...
  async function push(force, hand) {
    ...
    try {
      ...
      window.Lab.gizmos.forEach(g => {
        ...
        try { localStorage.removeItem(skey('pos:') + id); localStorage.removeItem(skey('z:') + id); } catch (e) {}
      });
