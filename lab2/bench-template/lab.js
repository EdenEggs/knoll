...
window.Lab = (function () {
  ...
  /* WHAT STANDS ON WHAT. Two numbers per feature, and only one of them is
     ...
     Ranks are saved — knoll-@@BENCH_KEY@@:z:<gizmo> on this machine, data-home-z in
  ...
  let menuOn = null;                     // the SUBJECT the right-click menu is about — see THE MENU
  ...
  /* AND THE WALL PUTS ITS OWN PICK DOWN WITH IT. There are two picks on this
     bench — the features, held here, and the marks on the paper, held in
     wall.js — but only ever ONE gesture that made them: the same band, the
     same escape, the same click on bare paper. So the two are cleared
     together, or a stray shift-click would leave crop marks standing over
     stickers this file has no idea about. (Wall may not be loaded yet: this
     runs during boot, and lab.js is the earlier script.) */
  function clearPick() {
    if (window.Wall && Wall.clearPick) Wall.clearPick();
  ...
  /* Let go of the features' clipboard. Called when the OTHER half of the
     paper was the one copied from — see the ctrl+c handler, which owns that
     decision because it is the only place that sees both answers. */
  const dropClip = () => { clip = null; pasted = 0; };
  ...
  function paste() {
    ...
    clip.forEach(c => {
      ...
      if (c.cut) { try { localStorage.setItem('knoll-@@BENCH_KEY@@:size2:' + e.id, JSON.stringify(c.cut)); } catch (err) {} }
  ...
  function unpaste(el) {
    ...
    pick(el, false);
    lifted.delete(el);
    if (menuOn && menuOn.el === el) closeMenu();
  ...
  function deleteCopy(el) {
    ...
    remember(() => {
      const back = makeGz(e);
      if (!back) return;
      if (cut) { try { localStorage.setItem('knoll-@@BENCH_KEY@@:size2:' + e.id, JSON.stringify(cut)); } catch (err) {} }
  ...
  /* ── TAKEN OFF THE PAPER ────────────────────────────────────────────────
     ...
     on a list so it stays off after a reload (knoll-@@BENCH_KEY@@:gone here,
  ...
  function shelve(el) {
    ...
    let size = null;
    try { size = JSON.parse(localStorage.getItem('knoll-@@BENCH_KEY@@:size2:' + el.dataset.gizmo)); } catch (e) {}
    const z = zOf(el);
  ...
  function unhide(r) {
    ...
    if (r.cut) { try { localStorage.setItem('knoll-@@BENCH_KEY@@:size2:' + r.id, JSON.stringify({ w: r.w, h: r.h })); } catch (e) {} }
    world.appendChild(el);
    register(el);
  ...
  const CAM_KEY = 'knoll-@@BENCH_KEY@@:cam', OLD_ZKEY = 'knoll-@@BENCH_KEY@@:zoom';
  ...
  /* EVERYTHING ON THE PAPER, and the features are only half of it. This is
     what the camera is clamped inside (clampCam) and what shift 1 frames
     (fit), and for most of this file's life it counted FEATURES — which on
     lab 2's paper is everything there is.

     On a bench that starts bare it is nothing at all, and the fallback took
     over: a rectangle the size of the viewport at world 0,0. Then every
     sticker stamped out at world 2000 was outside the only rectangle the
     camera believed in, so the clamp spent its time reeling the view back
     towards an empty sheet — which is what "it zooms to the middle" was.
     Zooming IN is where it showed, because the clamp is written in screen
     pixels and the sheet's edges run away from the middle as Z grows.

     So the wall is asked too. It answers with the upright box round every
     mark on it — stickers, tracings, gifs, strokes, notes — or nothing when
     the wall is empty, and the two boxes are one rectangle. The answer is
     cached in cBox like everything else here; wall.js drops that cache
     through Lab.forget whenever its store changes. */
  function contentBox() {
    ...
    gizmos.forEach(g => { if (!out(g.el)) eat(geoOf(g.el)); });
    const w = window.Wall && Wall.bounds && Wall.bounds();
    if (w) eat(w);
  ...
  function endSweep(e) {
    ...
    /* …AND THE OTHER HALF OF THE PAPER. The band is one rectangle over the
       whole bench, so it asks both of the things that can be on the bench
       what fell inside it: the features here, and the marks on the wall over
       there. On a bench with no features — the Iron Hive is one — the second
       line is the only one that ever finds anything. */
    if (window.Wall && Wall.band) Wall.band(x1, y1, x2, y2, was.add);
  }
  ...
  /* ── AND THE BROWSER'S OWN DRAG IS NOT WANTED ON THE PAPER ───────────────
     ...
     ends a band or a pan on pointercancel exactly as it does on pointerup:
     correctly, and in the wrong place. What that looked like was a band that
     stopped growing a few pixels in and picked up whatever happened to be in
     that first corner, with nothing on screen to say why.

     It needed a selection lying about to trigger, and one is easy to come by:
     any drag across the paper that is not a band drags through the words on
     the page behind it. Then the NEXT press anywhere is a press on a document
     that has something selected, and Chrome offers to drag it.

     Nothing on this bench ever starts a native drag on purpose. The tracing
     table takes files DROPPED on it (dragenter/dragover/drop in tracer.js)
     and that is inbound and untouched; a sticker carried out of the drawer
     and a gif carried off the row are pointer gestures, not dnd. So the
     outbound half is refused outright, which is one line and cannot be got
     wrong later. The other half of the answer is in lab.css: banding wears
     user-select:none the way panning always has, so the selection that starts
     all this is never made in the first place. (2026-09-08) */
  ...
  /* ── THE MENU ────────────────────────────────────────────────────────────
     ...
     IT IS ABOUT A SUBJECT AND NOT ALWAYS A FEATURE. On this bench the paper
     starts bare and everything you put on it is a mark on the wall, so a
     menu that only ever answered to `.gz` was a menu nobody could open. The
     three buttons ask a small object instead —

       { el?, name, where(), raise(), lower(), remove() }

     — and a feature is one of those, built by featureMenu below. Anything
     else that owns things on the paper hands lab.js a FINDER through
     Lab.onMenu: given whatever was right-clicked, answer with a subject or
     with null. wall.js registers one for the stickers (see ITS MENU over
     there), which is the same split the band already runs on — lab.js owns
     the gesture, the other file owns the answer.

     `el` is optional and is only what wears .menued: a wall item's node is
     thrown away and rebuilt on every repaint, so it says nothing and shows
     what the menu is about through the pick instead. */
  ...
  // a feature, as a subject. Null for anything this file has never heard of,
  // which is what sends the right-click on to the finders below.
  function featureMenu(el) {
    if (!el || !rec(el)) return null;
    return {
      el,
      name: el.getAttribute('aria-label') || el.dataset.gizmo,
      where: () => {
        if (!rec(el)) return '';
        const crew = picked.has(el) && picked.size > 1 ? picked.size + ' picked · ' : '';
        return crew + 'layer ' + (rankOf(el) + 1) + ' of ' + gizmos.length;
      },
      raise: () => raise(el),
      lower: () => lower(el),
      remove: () => remove(el)
  ...
  // what else on the paper can be right-clicked — wall.js puts its stickers here
  const finders = [];
  const onMenu = fn => { if (typeof fn === 'function') finders.push(fn); };

  function subjectAt(t) {
    if (!t || !t.closest) return null;
    const s = featureMenu(t.closest('.gz'));
    if (s) return s;
    for (let i = 0; i < finders.length; i++) {
      let f = null;
      try { f = finders[i](t); } catch (e) {}
      if (f) return f;
    }
    return null;
  }
  ...
  function sayWhere() {
    ...
    w.textContent = menuOn && menuOn.where ? (menuOn.where() || '') : '';
  }

  function showMenu(s, x, y) {
    closeMenu();
    if (!s) return;
    menuOn = s;
    if (s.el) s.el.classList.add('menued');
    menu.querySelector('.lab-menu-name').textContent = s.name || '';
    menu.hidden = false;
  ...
  // the feature door, kept for frames.js: it forwards a right-click it caught
  // inside a live document and knows which panel it came out of, not a subject
  const menuAt = (el, x, y) => showMenu(featureMenu(el), x, y);

  function closeMenu() {
    if (!menuOn) return;
    if (menuOn.el) menuOn.el.classList.remove('menued');
    menuOn = null;
    menu.hidden = true;
  }
  ...
  menu.addEventListener('click', e => {
    ...
    if (!b || !menuOn) return;
    const s = menuOn;
    if (b.dataset.act === 'raise') { s.raise(); sayWhere(); }
    else if (b.dataset.act === 'lower') { s.lower(); sayWhere(); }
    else if (b.dataset.act === 'remove') { closeMenu(); s.remove(); }
  });
  ...
  bench.addEventListener('contextmenu', e => {
    ...
    /* AIMED BY THE PRESS THAT OPENED IT, and not by its own target. A
       contextmenu event reports its point as WHOLE PIXELS while the press
       before it reports fractions: right-click a sticker at 559.85 and the
       menu arrives aimed at 559, and on artwork as thin as a chain link that
       eighth of a pixel is regularly a hole — Chrome then hands over the DIV
       under the ink layer rather than the piece, and the sticker appears to
       have no menu at all. The press is the same gesture at the same instant
       and it is the hit everything else on this paper is already decided by,
       so it goes first. A menu asked for from the KEYBOARD has no press
       behind it, which is what the second line is for. */
    const s = subjectAt(aimed(e)) || subjectAt(e.target);
    if (!s) return;
    e.preventDefault();
    showMenu(s, e.clientX, e.clientY);
  });
  /* the press a right-click was made with, kept for the contextmenu that
     follows it — see AIMED BY THE PRESS above. Held only as long as the two
     agree about where they are, so a menu opened any other way falls back to
     its own target rather than to a stale one. */
  let pressed = null;
  const aimed = e => (pressed && Math.abs(pressed.x - e.clientX) < 2 &&
                                 Math.abs(pressed.y - e.clientY) < 2) ? pressed.t : null;

  document.addEventListener('pointerdown', e => {
    pressed = e.button === 2 ? { t: e.target, x: e.clientX, y: e.clientY } : null;
    if (menuOn && !(e.target.closest && e.target.closest('#lab-menu'))) closeMenu();
  }, true);
  ...
  document.addEventListener('keydown', e => {
    ...
    if ((e.ctrlKey || e.metaKey) && !e.altKey) {
      ...
      /* …and only swallowed when they DID something: with nothing picked and
         ...
         BOTH HALVES OF THE PAPER ARE ASKED, the features here and the marks
         on the wall over there, because one band can pick up both and a
         person who selected both meant both. Each keeps its own clipboard and
         each empties the other's on a successful copy (Lab.dropClip,
         Wall.dropClip), so ctrl+v never pastes two unrelated lots at once. On
         a bench with no features — the Iron Hive is one — the second call is
         the only one that ever does anything. */
      else if (e.key.toLowerCase() === 'c') {
        const gz = copy(), wall = window.Wall && Wall.copy ? Wall.copy() : false;
        // …and whichever half was NOT copied from lets go of what it was
        // holding, since the two are one clipboard between them. Decided
        // here rather than inside either: this is the only place that can
        // see both answers, and each copying itself would leave the second
        // one to run having just wiped the first.
        if (gz && !wall && window.Wall && Wall.dropClip) Wall.dropClip();
        if (wall && !gz) dropClip();
        if (gz || wall) e.preventDefault();
      }
      else if (e.key.toLowerCase() === 'v') {
        const gz = paste(), wall = window.Wall && Wall.paste ? Wall.paste() : false;
        if (gz || wall) e.preventDefault();
      }
      return;
    }
  ...
  return { register, place, resetAll, gizmos, store, uid, resetData, world, bench, grid, growBench,
    ...
    /* IS THE MENU UP? A getter, because menuOn is a live variable and a
       ...
       before it lets escape put the pen down: with a menu open, escape is
       the menu's and nothing else's. */
    get menuUp() { return !!menuOn; },
