/* seed.js — THE WALL THIS BENCH OPENS ON, and SAVE, which writes it.
   (lab2/bench-template; the same file the iron hive carries.)

   Everything on the paper lives in the browser's localStorage, per site and
   per browser, so a page would open on bare paper for everyone but the
   person who arranged it. So the wall ships beside the page: wall-seed.json
   holds the pieces, the tracings some of them are stamped from, and the
   camera, and a browser that has never had a wall of its own here opens on
   it. On a new bench the file is empty, and so is the paper.

   ONCE, AND ONLY WHERE THERE IS NOTHING. The seed goes on the first time this
   browser comes to the page — no wall store yet, or one with nothing live on
   it, and no mark from an earlier visit — and what the visitor does after
   that is theirs and is never written over. "reset data" wipes the paper and
   leaves the mark standing: a reset means bare paper.

   READ BEFORE THE STORES OPEN, which is why this file is loaded straight
   after lab.js: Lab.store writes a fresh, empty store the moment it is
   opened, so whether this browser HAD a wall is read before wall.js and
   tracer.js run.

   SAVING IS THE OTHER HALF, and the dev server's alone — see SAVE, at the foot
   of this file. A camera goes with a saved wall as the world point under the
   middle of the bench, the zoom, and the bench's width; a bench under 700 wide
   keeps a phone framing of its own (cam_narrow). */
window.Seed = (function () {
  const KEY = { wall: 'knoll-@@BENCH_KEY@@:wall', flatfile: 'knoll-@@BENCH_KEY@@:flatfile', cam: 'knoll-@@BENCH_KEY@@:cam', mark: 'knoll-@@BENCH_KEY@@:seeded' };
  const FILE = 'wall-seed.json', DOOR = '/_@@BENCH_SLUG@@/wall';
  ...
  /* ── SAVE: THE PAPER, WRITTEN INTO THE SITE (2026-09-12) ──────────────────
     ...
     it stands goes into @@BENCH_SLUG@@/wall-seed.json, the file every first visit opens
  ...
  const OLD = 'Not saved: the server running this page is older than its save button. ' +
              'Close that server’s window, start it again (node serve.js), and save again.';
  const TITLE = 'save the paper into the site (ctrl+s): every piece as it is now becomes what this bench opens on. ' +
  ...
  function show(state, text, note, bad) {
    ...
    btn.title = TITLE;
    const n = document.getElementById('bench-save-note');
  ...
  async function save(withView) {
    ...
    if (!why) {
      ...
      show('done', 'saved ✓', withView ? 'Saved, with this view as the one ' + (post.which === 'narrow' ? 'a phone opens on.' : 'this bench opens on.') : '');
  ...
  seedP.then(seed => ready(() => {
    ...
    btn = document.getElementById('bench-save');
    if (!btn) return;
