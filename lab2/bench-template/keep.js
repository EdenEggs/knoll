/* ── KEEP: THE BENCH SAVES ITSELF AS THE DEFAULT LOOK ──────────────────────
   ...
   knoll-@@BENCH_KEY@@:pos:<gizmo> and frames.js keeps re-cut sizes in
   knoll-@@BENCH_KEY@@:size2:<gizmo>, and both of those are IN THIS BROWSER — they are
...
window.Keep = (function () {
  ...
  const DOOR = '/_@@BENCH_SLUG@@/default';
  const EVERY = 30000;
  ...
  async function push(force, hand) {
    ...
    try {
      ...
      window.Lab.gizmos.forEach(g => {
        ...
        try { localStorage.removeItem('knoll-@@BENCH_KEY@@:pos:' + id); localStorage.removeItem('knoll-@@BENCH_KEY@@:z:' + id); } catch (e) {}
      });
