...
window.Kits = (function () {
  ...
  // ── boot ─────────────────────────────────────────────────────────────────
  convertAll();
  // the sheets are asked for NOW, while the rest of the page is still
  ...
  // later
  /* ONLY THE SHEETS THE PAPER ASKS FOR, which on lab 2 is all three and on
     this bench is none. The eagerness above is paid for by index.html's
     preloads; an empty bench has neither the preloads nor a section naming a
     sheet, and fetching all three anyway is three 404s at boot for art that
     is not copied here. convertAll() has just run, so a kit section carries
     its data-kit — but data-src is asked for too, so this is right whether or
     not it has. Put a sheet in features/, add a section naming it, and it is
     fetched exactly as before. */
  const wantedKits = () => [...new Set(
    [...document.querySelectorAll('#bench .gz, #bench section')]
      .map(el => (el.dataset && (el.dataset.kit || (kindOf(el.dataset.src) || {}).kit)) || null)
      .filter(k => k && SHEETS[k]))];
  wantedKits().forEach(loadSheet);
  ...
  function start() {
    ...
    // the same question as at boot, asked again now every section is adopted
    const WANT = wantedKits();
    Promise.all(WANT.map(loadSheet)).then(() => { layoutTiles(); relive(); remust(); });
    ...
    WANT.forEach(k => loadSheet(k).then(() => setTimeout(flushInk, 400)));
