...
window.Lab = (function () {
  ...
  /* WHAT STANDS ON WHAT. Two numbers per feature, and only one of them is
     ...
     Ranks are saved — knoll-toem2:z:<gizmo> on this machine, data-home-z in
  ...
  function paste() {
    ...
    clip.forEach(c => {
      ...
      if (c.cut) { try { localStorage.setItem('knoll-toem2:size2:' + e.id, JSON.stringify(c.cut)); } catch (err) {} }
  ...
  function deleteCopy(el) {
    ...
    remember(() => {
      const back = makeGz(e);
      if (!back) return;
      if (cut) { try { localStorage.setItem('knoll-toem2:size2:' + e.id, JSON.stringify(cut)); } catch (err) {} }
  ...
  /* ── TAKEN OFF THE PAPER ────────────────────────────────────────────────
     ...
     on a list so it stays off after a reload (knoll-toem2:gone here,
  ...
  function shelve(el) {
    ...
    let size = null;
    try { size = JSON.parse(localStorage.getItem('knoll-toem2:size2:' + el.dataset.gizmo)); } catch (e) {}
    const z = zOf(el);
  ...
  function unhide(r) {
    ...
    if (r.cut) { try { localStorage.setItem('knoll-toem2:size2:' + r.id, JSON.stringify({ w: r.w, h: r.h })); } catch (e) {} }
    world.appendChild(el);
    register(el);
