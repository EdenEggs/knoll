...
window.Tape = (function () {
  ...
  const store = Lab.store('tape', () => ({
    /* NO STRIP TO START WITH, ON THIS BENCH (2026-09-12). Lab 2 parks one in
       the gutter between its first two rows of features, at world 60–700 ×
       ~1100 — which on this bench runs straight across Town Square, the first
       of the eight plates the wall opens on, and this page was asked for
       nearly empty. The dock's tape button still strings one across the
       middle of whatever you are looking at (add(), below). */
    strips: []
  }));
  ...
  function grab(e) {
    ...
    const up = () => {
      ...
      /* …and undo is told how to put it back. BOTH ENDS, whichever handle was
         pulled: where a strip is and how long and which way round it lies are
         one fact with four numbers in it, and half of it restored is a strip
         nobody left anywhere. s0 is those four from before the drag, caught
         above; the strip is found by id rather than by index, so a strip
         dropped in between makes this a no-op instead of moving the wrong
         one. A drag that went nowhere files nothing. */
      if (now.x1 !== s0.x1 || now.y1 !== s0.y1 || now.x2 !== s0.x2 || now.y2 !== s0.y2) {
        const back = { x1: s0.x1, y1: s0.y1, x2: s0.x2, y2: s0.y2 };
        Lab.remember(() => store.update(st => {
          const s = st.strips.find(v => v.id === id);
          if (s) Object.assign(s, back);
        }));
      }
    };
    ...
    e.preventDefault();
    /* the bench must not pan under it. This alone does not do that: the
       camera listens on the CAPTURE phase, so by the time this runs the pan
       is already armed, and it is livePan that then swallows the moves on
       their way down — camera slides, tape stands still, which is exactly
       what it did. What keeps the bench still is .tape being named in
       onPaper and HANDS_OFF in lab.js; this only stops the rest of the
       bubble. Take the tape out of those two lists and all three drags die. */
    e.stopPropagation();
  }
